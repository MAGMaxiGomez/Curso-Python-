...
def elegir_palabra(lista_palabras):
    ...
    letra_unicas = len(set(palabra_elegida))

    return palabra_elegida, letra_unicas

def pedir_letra():
    letra_elegida = ""
    es_valida = False
    abcedario = "abcdefghijklmnñopqrstuwxyz"

    while not es_valida:
        letra_elegida = input("Elegi una letra: ").lower()
        if letra_elegida in abcedario and len(letra_elegida) == 1:
            es_valida = True
        else:
            print("No has elegido una letra incorrecta.")
